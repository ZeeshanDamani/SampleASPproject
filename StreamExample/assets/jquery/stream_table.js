...
(function (window, $) {
    ...
    _F.fetchData = function () {
        ...
        //this.opts.data_url = this.opts.data_url.split('?')[0];
        if (this.opts.queryString) {
            var newParams = this.opts.queryString.split('&');
            if (newParams) {
                for (var p = 0; p < newParams.length; p++) {
                    params[newParams[p].split('=')[0]] = newParams[p].split('=')[1]
                }
            }
        }
        var par = convArrToObj(params);
        params = JSON.stringify(par);
        $.ajax({
            type: "POST",
            url: this.opts.data_url,
            //data: "{'limit':'100' ,'offset':'" + this.data.length + "'}",
            data: params,
...
var isThreadUpdateRequired = false;
function checkForThreadUpdate() {
    if (isThreadUpdateRequired) {
        updateThreadGrid();
    }

}
var isVoidLabelUpdateRequired = false;
function checkForLabelUpdate() {
    if (isVoidLabelUpdateRequired) {
        updateLabelStatus();
        updateLabelReason();
    }
}

//var ispoUpdateRequired = false;
//function checkForPOUpdate() {
//    if (ispoUpdateRequired) {
//        updatePOGrid();
//    }
